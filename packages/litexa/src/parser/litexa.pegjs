{
  ...
  */
  ...
  var pushIntent = function(location, utterance, intentInfo, fromAlternate) {
    ...
    let intent = null;
    if (stack[0].target && stack[0].target.pushIntent) {
      intent = stack[0].target.pushIntent(stack[1].target, location, utterance, intentInfo);
      popStackTo(1);
      pushToStack(intent);
    } else {
      // during testing there is no skill, just construct the intent
      intent = new lib.Intent({location: location, utterance: utterance});
    }
    return intent;
  }
...
AlternativeStatement
  = "or" ___ '"' parts:SayStringContents '"' {
    const target = getTarget();
    if (target && target.pushAlternate) {
      if (target instanceof lib.Intent) {
        let group = new lib.PartListPart(location(), parts);
        let utterance = new lib.Utterance(location(), group);
        target.pushAlternate(location(), utterance, skill);
      } else {
        target.pushAlternate(location(), parts, skill);
      }
    } else {
      throw new ParserError(location(), "Couldn't find anything to add an alternative to here");
    }
  }
  / "or" ___ utterance:UtteranceString {
    const target = getTarget();
    if (target && target.pushAlternate) {
      target.pushAlternate(location(), utterance, skill);
    } else {
  ...
  / "or" ___ parts:(AssetName / DottedIdentifier) {
    const target = getTarget();
    if (target instanceof lib.Intent) {
      if (parts.isAssetName) {
        // parts was mistakenly parsed into an AssetName type,
        // reconstruct the original text and treat it as an alternative intent name instead
...
UtteranceString
  = '"' parts:UtteranceStringAlternation '"' {
    return new lib.Utterance(location(), parts);
  }

UtteranceStringAlternation
  = a:UtterancePartGroup tail:UtteranceStringAlternationTail+ {
    tail.unshift(a);
    return new lib.AlternatingPart(location(), tail);
  }
  / UtterancePartGroup

UtteranceStringAlternationTail
  = '|' a:UtterancePartGroup { return a; }
  / '|' { return new lib.StringPart(''); }

UtterancePartGroup
  = parts:UtteranceStringPrimitive+ {
    if (parts.length == 1) {
      return parts[0];
    }
    return new lib.PartListPart(location(), parts);
  }

UtteranceStringPrimitive
  = $(!'"' !'$' !'(' !')' !'|' .)+ { return new lib.StringPart(text()); }
  / name:SlotVariableReference { return new lib.SlotReferencePart(name); }
  / '(' contents:UtteranceStringAlternation ')' { return contents; }
...
*/
SayString
  = '"' parts:SayStringContents '"' {
    return new lib.Say(parts, skill);
  }

SayStringContents
  = part:SayStringAlternation { return [part]; }

SayStringAlternation
  = a:SayStringGroup tail:SayStringAlternationTail+ {
    tail.unshift(a);
    return new lib.AlternatingPart(location(),tail);
  }
  / SayStringGroup

SayStringAlternationTail
  = '|' a:SayStringGroup { return a; }
  / '|' { return new lib.StringPart('') }
...
SayStringPart
  ...
  / TemplateStringTaggedPart
  / '(' contents: SayStringAlternation ')' { return contents; }
